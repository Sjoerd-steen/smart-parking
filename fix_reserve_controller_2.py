...
class ReservationController extends Controller {
    ...
    private function updateExpiredReservations() {
        ...
        Reservation::where('status', 'actief')
            ->where(function($query) use ($currentDate, $currentTime) {
                $query->where('datum', '<', $currentDate)
                      ->orWhere(function($q) use ($currentDate, $currentTime) {
                          $q->where('datum', '=', $currentDate)
    ...
    public function store(Request $request) {
        ...
        $existing = Reservation::where('external_parking_id', $request->external_parking_id)
            ...
            ->where(function($query) use ($request) {
                $query->where('start_tijd', '<', $request->eind_tijd)
                      ->where('eind_tijd', '>', $request->start_tijd);
            })
            ->exists();
        if ($existing) {
             return redirect()->route('user.reserve')->with('error', 'Deze parkeerplaats is al gereserveerd voor deze tijd.');
        }
